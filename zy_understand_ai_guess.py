...
#     return -(y - y_hat) * y_hat * (1 - y_hat)
    return np.dot(x.reshape(4, 1), (y_hat - y).reshape(1, 3))
...
#             print((np.dot(x,weights)).shape) (3,)
#             print(x.shape) (4,)
            ...
            #print(weights.shape)
            del_w -= error_term
# del_w -= error_term 与上面 return np.dot(x.reshape(4, 1), (y_hat - y).reshape(1, 3))搭配
# del_w += error_term 与上面 return np.dot(x.reshape(4, 1), (y - y_hat).reshape(1, 3))搭配
...
print(np.equal(predictions, np.argmax(np.array(test_label), axis=1)))
print(np.mean(np.equal(predictions, np.argmax(np.array(test_label), axis=1))))
...
# predict()函数返回的是经过 sofamax 运算后具有最大值的列index
...
# with open('images/validations.csv'):
...
print("^^^")
print(val_data['feature2'])
print(val_data['feature4'])
...
for i in range(features.shape[0]):
    print(id_to_obj[predict(features[i])])
    print(predict(features[i]))
    print( "The {}th image is {}".format(i, id_to_obj[predict(features[i])]))  
# predict()函数返回列index，在id_to_obj 数组中找到对应结果。
